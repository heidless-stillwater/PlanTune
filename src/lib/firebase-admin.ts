...
/**
 * Proxied Firebase services — ensures initialization on any access.
 */
...
// PlanTune's own database
export const adminDb: any = new Proxy({} as any, {
    get(target, prop) {
        const app = getEnsuredApp();
        const databaseId = process.env.NEXT_PUBLIC_FIREBASE_DATABASE_ID || process.env.FIREBASE_DATABASE_ID || 'plantune-db-0';
...
// Identity store (read-only for entitlement checks)
...
// Compliance hub (read-only for sovereign gating)
