...
import { ChevronDown, LayoutGrid, Zap, Book, Shield, Video, PlayCircle, TrendingUp } from 'lucide-react';

const apps = [
    { name: 'PlanTune', url: 'http://localhost:3004', icon: TrendingUp, color: 'text-teal-400', desc: 'Credit Strategy Engine', current: true },
    ...
    return (
        <div className="relative" ref={ref}>
            <button onClick={() => setIsOpen(!isOpen)}
                ...
                <LayoutGrid size={16} className="text-[var(--foreground-muted)] group-hover:text-white" />
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--foreground-muted)] group-hover:text-white hidden sm:block">Suite</span>
                <ChevronDown size={14} className={`text-[var(--foreground-muted)] transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
            </button>
            {isOpen && (
                    ...
                    <div className="grid grid-cols-1 gap-1">
                        {apps.map((app) => (
                            <a key={app.name} href={app.url}
                               className={`flex items-center gap-4 p-3 rounded-xl hover:bg-white/5 transition-all group ${(app as any).current ? 'bg-white/5 border border-white/10' : ''}`}>
